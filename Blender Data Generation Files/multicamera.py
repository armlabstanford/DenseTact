...
#               },
               {"name": "camera_1", "angle_x":180, "fl_x":25, "near":0.0001,
                ...
                "radius": .125, "zcutoff": None, "num_views": 200,
               ...
               {"name": "camera_2", "angle_x":60, "near":.1,
                ...
                "radius": 0.5, "zcutoff": None, "num_views": 100,
...
#               },
               ...
               {"name": "camera_7", "angle_x":30, "near":0.1,
                "far":1.5, "res_x": 1024, "res_y": 1024,
                "radius": 1, "zcutoff": None, "num_views": 100,
...
#                    camera = cameras[0]
#                    print(camera["name"])
#                    out_data[ctype][type]["camera_angle_x"] =  2*math.atan(camera["res_x"] /(2*bpy.data.objects[camera["name"]].data.lens))
#                    out_data[ctype][type]["fl_x"] = bpy.data.objects[camera["name"]].data.lens
#                    out_data[ctype][type]["near"] = bpy.data.objects[camera["name"]].data.clip_start
#                    out_data[ctype][type]["far"] = bpy.data.objects[camera["name"]].data.clip_end
#
...
#            stop
#            points = gen_viewpoints(camera)
#            focal_lengths = [bpy.data.objects[camera["name"]].data.lens]*len(points)
#            x_resolutions = [camera["res_x"]]*len(points)
#            y_resolutions = [camera["res_y"]]*len(points)
#            cam_name = [camera["name"]]*len(points)
#        else:
#            out_data["cameras"] = []
#            for camera in cameras:
#                cam = {
...
#                        "w": camera["res_x"],
#                        "h": camera["res_y"],
#                        "camera_angle_x": 2*math.atan(camera["res_x"] /(2*bpy.data.objects[camera["name"]].data.lens)),
#                        "fl_x": bpy.data.objects[camera["name"]].data.lens,
#                        "near": bpy.data.objects[camera["name"]].data.clip_start,
#                        "far": bpy.data.objects[camera["name"]].data.clip_end,
#                      }
#                out_data["cameras"].append(cam)
#                cam_points = gen_viewpoints(camera)
#                points = points + cam_points
#                focal_lengths = focal_lengths + [bpy.data.objects[camera["name"]].data.lens]*len(cam_points)
#                x_resolutions = x_resolutions + [camera["res_x"]]*len(cam_points)
#                y_resolutions = y_resolutions + [camera["res_y"]]*len(cam_points)
#                cam_name = cam_name + [camera["name"]]*len(cam_points)
#
#        inds = np.arange(0, len(x_resolutions))
#        inds = np.random.permutation(inds)
#        points = list(np.asarray(points)[inds])
#        x_resolutions = list(np.asarray(x_resolutions)[inds])
#        y_resolutions = list(np.asarray(y_resolutions)[inds])
#        cam_name = list(np.asarray(cam_name)[inds])
#
#        for i, loc in enumerate(points):
#
#            print("using camera:")
...
#            scene.render.resolution_percentage = 100
#
#            bpy.data.objects[cam_name[i]].select_set(True)
#            cam = scene.objects[cam_name[i]]
#            bpy.context.scene.camera = cam
#
#            print('Iteration', i)
#            cam.location = loc
...
#                    elif ctype == 'depth' or 'touch':
#                        scene.render.filepath = os.path.join(fp, ctype, type, 'r_' + str(i))
#                        get_depth_image(cam_name=cam_name[i], b_invert=False, save_fn=os.path.join(fp, ctype, type, 'r_' + str(i)+'.npy'))
#
#
#            frame_data = {
#                    'camera': cam_name[i],
...
#            gc.collect()
#
#            bpy.data.objects[cam_name[i]].select_set(False)
#
#
#        for ctype in CAMERA_TYPE:
#            for type in TRAIN_TYPE:
#                with open(os.path.join(fp, ctype, type, f'transforms_{type}.json'), 'w') as out_file:
...
#print("CHECKING CAMERA ANGLE")
##print(bpy.data.objects[camera].data.angle_x)
#
##math.atan(RESOLUTION_X/(2*bpy.data.objects[camera].data.lens/1000))
##print("HELLO!")
##print(bpy.data.objects[camera].data.lens)
##print("GOOD DAY")
##print(360/(2*np.pi)*2*math.atan(RESOLUTION_X /(2*bpy.data.objects[camera].data.lens)))
##print("FAIR WELL")
#out_data = {
#    'camera_angle_x': 2*math.atan(RESOLUTION_X /(2*bpy.data.objects[camera].data.lens)), #2*math.atan( RESOLUTION_X / 2*(bpy.data.objects[camera].data.lens/1000)), #bpy.data.objects[camera].data.angle_x,
#    'fl_x': bpy.data.objects[camera].data.lens,
#    'near': bpy.data.objects[camera].data.clip_start,
#    'far': bpy.data.objects[camera].data.clip_end,
#}
#
## Data to store in JSON file
#depth_out_data = {
...
#
##points = fibonacci_sphere(r=radius, samples=VIEWS)
#far = bpy.data.objects[camera].data.clip_end
#points = local_look(radius=.05,height=radius,phi=np.pi/14,theta=np.pi/16,psi=0, samples=VIEWS)
#
#fig = plt.figure()
#ax = fig.add_subplot(projection='3d')
#ax.set_aspect("equal")
...
#points = list(map(tuple, points.T))
#
##points_2 = fibonacci_sphere(r=2*radius, samples=VIEWS)
##points_3 = fibonacci_sphere(r=4*radius, samples=VIEWS)
##points = cube_look(r=radius, samples=int(VIEWS))
##points_2 = cube_look(r=radius+radius/2, samples=int(VIEWS/2))
...
#    bpy.ops.render.render(write_still=True)  # render still
#    print("fp")
#    print(os.path.join(fp,'{RESULTS_PATH}', 'r_' + str(i)+'.npy'))
...
#    json.dump(out_data, out_file, indent=4)
#
#with open(os.path.join(fp,RESULTS_PATH, f'transforms_{RESULTS_PATH}.json'), 'w') as out_file:
